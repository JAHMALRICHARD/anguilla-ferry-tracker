...
export function FerryDetailsCard({ ferry }: FerryDetailsCardProps) {
  ...
  const [depHours, depMinutes] = departureTime
    .replace(/(AM|PM)/i, '')
    .split(':')
    .map(Number)

  const isPM = departureTime.toLowerCase().includes('pm')
  const totalMinutes = (isPM && depHours !== 12 ? depHours + 12 : depHours) * 60 + depMinutes + 30
    ...
    <div className="w-full max-w-5xl bg-[#151923] text-white rounded-xl shadow-md p-6 space-y-6">
      <div className="flex items-center justify-between border-b border-gray-700 pb-4">
        <div className="flex items-center gap-4">
          <Image src={logoUrl} alt={`${operator} logo`} width={48} height={48} className="rounded-full bg-white p-1" />
  ...
  <p className="text-gray-400 text-xs mt-1 tracking-wide">ONE WAY <span className="inline-block">&rarr;</span></p>
</div>
